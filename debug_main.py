...
def train_epoch(trainload, model, opti, lr_sh, gpu=0):
    ...
    for i, sample in enumerate(progress):
        try:
            ...
            if "CUDA out of memory" in str(e):
                logging.error("CUDA out of memory during training, skipping batch.")
...
def test_epoch(testload, model, need, gpu=0):
    ...
    with torch.no_grad():
        for i, sample in enumerate((tqdm(testload))):
            try:
                ...
                if "CUDA out of memory" in str(e):
                    logging.error("CUDA out of memory during testing, skipping batch.")
                    torch.cuda.empty_cache()
                else:
                    raise e
    end_time = time.time()
...
def main(sample_path, split_path=None, batch=8, epoch=10, gpu=0, model_path=None, save_path=None):
    """Main training and evaluation function."""
    initialize_visualization()

    trainload = load_data(sample_path, split_path, batch, topk=3)
    testload = load_data(sample_path, split_path, batch, mode='test', topk=3)
...
if __name__ == '__main__':
    ...
    _ = main(sample_path, split_path, batch=16, epoch=10, save_path=save_path, model_path=None)
